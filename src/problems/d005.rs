    ...
    fn do_p1(&mut self) {
        ...
        for order in orders.lines() {
            let binding = order
                ...
                .replace("from ", "")
                .replace("to ", "");
            let order: Vec<&str> = binding.split(" ").collect();
            let (quantity, source, target) = (
                order[0].parse::<usize>().unwrap(),
                order[1].parse::<usize>().unwrap(),
                order[2].parse::<usize>().unwrap(),
            );
...
fn get_stacks(stack_input: &str) -> Vec<Vec<char>> {
    ...
    let mut rows: Vec<Vec<char>> = stack_input
        .lines()
